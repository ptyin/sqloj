...
public class SqlExecutionUtils
{
                             ...
                             List<String> statements)
    {
        ...
        if (sb.length() > 0 && containsText(sb))
        {
            statements.add(sb.toString());
        }
    }
...
//        var results = new ArrayList<QueryResult>();
        try
        {
            for (String statement : statements)
            {
                ...
                {
                    throw new InterruptedException(String.format("Interruption before statement [%d]: %s",
                            stmtNumber, stmt));
                }
                if(log.isDebugEnabled())
                    log.debug(String.format("Statement [%d]: %s", stmtNumber, statement));
                try
                {
                    long statementStart = ManagementFactory.getThreadMXBean().getCurrentThreadCpuTime();
                    ...
                    if (stmt.execute(statement))
                    {
                        ...
                        do
                        {
                            QueryResult queryResult = new QueryResult(stmt.getResultSet());
