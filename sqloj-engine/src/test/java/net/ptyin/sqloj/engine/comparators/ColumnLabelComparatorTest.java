...
class ColumnLabelComparatorTest
{
    ...
    void compare() throws Exception
    {
        ...
        ExecutionResult criterion = SqlExecutionUtils.execute(connection,
                "select title, albumid, artistid from albums;");
        ExecutionResult a = SqlExecutionUtils.execute(connection,
                "select albumid, artistid, title from albums; ");
        ExecutionResult b = SqlExecutionUtils.execute(connection, "select artistid, title from albums;");
        ExecutionResult c = SqlExecutionUtils.execute(connection, "select * from albums;");
        assertTrue(comparator.compare(a, criterion, new HashMap<>()));
        HashMap<String, Object> resultB = new HashMap<>();
